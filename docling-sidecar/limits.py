...
The docling converter parses attacker-supplied documents (PDF/Office/HTML/images
…), running layout and OCR models over them. A decompression bomb or a
pathological file can otherwise pin CPU or balloon memory inside the long-lived
uvicorn process. Running each conversion in a disposable child means:
...
def _context():
    ...
    # threadpool thread, and docling does lazy imports (and loads its layout/
    # OCR models) while converting, so a 'fork' here could deadlock the child
    # on an import/alloc lock held by another thread at fork time. The cost is
    # re-loading the converter's models per conversion — real, but accepted
    # for this low-QPS, safety-first document path (same trade-off as the
    # markitdown sidecar).
