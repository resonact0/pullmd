...
export function createAuth({ db, mode = 'disabled', env = {}, argon2Opts } = {}) {
  ...
  function mountAuthRoutes(app) {
    ...
    app.get('/login', (req, res) => {
      ...
      res.send(loginPage({ next: req.query.next || '' }));
    });
    ...
    app.post('/login', formParser, async (req, res) => {
      ...
      if (!user) {
        ...
        return res.send(loginPage({ error: 'Wrong email or password.', email, next }));
      }
    ...
    if (mode === 'multi-user') {
      ...
      app.post('/signup', formParser, async (req, res) => {
        ...
        const errs = (msg) => {
          ...
          return res.send(signupPage({ error: msg, email }));
        };
        if (password !== password_confirm) return errs('Passwords do not match.');
        if (typeof password !== 'string' || password.length < 8) return errs('Password must be at least 8 characters.');
        if (!validateEmail(email)) return errs('Invalid email.');
        const exists = db.prepare("SELECT 1 FROM users WHERE email = ?").get(validateEmail(email));
        if (exists) return errs('That email is already registered.');
        ...
        try {
          ...
          return errs(err.message || 'Could not create account.');
        }
      });
    }
    ...
    app.get('/settings', requireAuth(), (req, res) => {
      const newKey = req.query.new_key && typeof req.query.new_key === 'string'
        ? req.query.new_key : null;
      res.set('Content-Type', 'text/html; charset=utf-8');
    ...
    app.post('/api/keys', formParser, requireAuth(), (req, res) => {
      ...
      res.redirect(302, '/settings?new_key=' + encodeURIComponent(fullKey));
    });
