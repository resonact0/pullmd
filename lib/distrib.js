...
/** Read public/help.html from disk and substitute the placeholder. */
...
export function renderHelp(baseUrl) {
  return substituteUrl(helpTemplate(), baseUrl);
}
 ...
 *   web-reader/skills/web-reader.md
...
export function buildSkillZip(baseUrl) {
  return new Promise((resolve, reject) => {
    ...
    for (const rel of SKILL_FILES) {
      ...
      archive.append(substituteUrl(content, baseUrl), { name: `web-reader/${rel}` });
    }
    archive.finalize();
  });
}
