...
async function convertWithReadability(url, html, comments, statusCode, fetchFn, extractor) {
  const cleanedHtml = preprocess(html);
...
export async function extractWeb(url, options = {}) {
  ...
  // extractor=playwright implies forced render — Playwright must run.
  const effectiveRender = extractor === 'playwright' ? 'force' : render;
  ...
  const headers = { 'User-Agent': pickUserAgent() };
  if (!comments) {
  ...
  const result = await convertWithReadability(url, body, comments, statusCode, rawFetchFn, extractor);
  ...
  try {
    ...
    const rendered = await convertWithReadability(url, renderedHtml, comments, statusCode, rawFetchFn, extractor);
    rendered.source = 'playwright';
    const renderReason = extractor === 'playwright'
