...
import { convertViaDocling } from './docling-client.js';
import { convertMediaViaWhisper } from './whisper-media-client.js';
 ...
 *
 * Falls back to the extension whenever the content-type isn't itself a
 * recognized image/audio type — not just when it's empty/octet-stream.
 * Browsers are inconsistent about audio MIME types (e.g. .m4a is commonly
 * reported as audio/x-m4a, which isn't in AUDIO_CONTENT_TYPES), so trusting
 * only the whitelist silently drops uploads to the markitdown fallback.
 */
...
function removeRecipeSelectors(document, selectors) {
  for (const sel of selectors) {
    try {
...
async function convertWithReadability(url, html, comments, statusCode, fetchFn, extractor, recipe, filename) {
  ...
  // recomputes them from the rendered DOM. Never throws out into extraction.
  if (recipe?.frontmatter?.fields) {
    try {
      const recipeFields = extractRecipeFrontmatter(cleanedHtml, recipe.frontmatter);
      if (recipeFields && Object.keys(recipeFields).length > 0) {
        metadata.recipeFields = recipeFields;
      }
  ...
  // sidecar, which receives cleanedHtml. Previously cleanedHtml was serialized
  // before removal, making select.remove a silent no-op whenever the quality
  // auto-pick chose Trafilatura. Deliberately runs AFTER metadata + recipe
  // frontmatter extraction (documented: frontmatter selector sources see the
  // page before boilerplate stripping) and BEFORE the Trafilatura call.
  if (recipe?.removeSelectors?.length) {
    removeRecipeSelectors(document, recipe.removeSelectors);
    cleanedHtml = document.toString();
  }
  ...
  if (article && article.content && article.content.length >= 200) {
    ...
    // cleanedHtml already has recipe removeSelectors baked in (see above).
 ...
 * @param {'docling'} [options.engine]  Opt-in high-quality/complex-document engine for linked documents (query param)
 * @param {(buf: Buffer, opts: object) => Promise<{markdown:string,title:string|null}|null>} [options.doclingClient]  Defaults to convertViaDocling; injectable for tests
 * @param {'whisper'} [options.mediaEngine]  Force yt-dlp+Whisper media ingestion for non-YouTube URLs (query param); also used automatically as the YouTube no-caption fallback
 * @param {(url: string, opts: object) => Promise<{markdown:string,title:string|null,fields?:object,transcriptStatus?:string}|null>} [options.whisperMediaClient]  Defaults to convertMediaViaWhisper; injectable for tests
...
export async function extractWeb(url, options = {}) {
  const {
    ...
    engine,                                 // 'docling': opt-in high-quality/complex-document engine (query param)
    doclingClient = convertViaDocling,      // injectable for tests
    mediaEngine,                            // 'whisper': force yt-dlp+Whisper media ingestion (query param)
    whisperMediaClient = convertMediaViaWhisper, // injectable for tests
    ytTimecodes,
    ytChunk,
  } = options;
  ...
  // Opt-in high-quality/complex-document engine (?engine=docling). Falls
  // back to markitdown below on no-sidecar (null) or conversion failure.
  if (!comments && engine === 'docling' && isMarkitdownTarget(ct, url) && bodyBytes != null) {
    let filename;
    try { filename = new URL(url).pathname.split('/').pop() || undefined; } catch { filename = undefined; }
    let docling = null;
    let doclingErr = null;
    try { docling = await doclingClient(bodyBytes, { filename, contentType: ct, signal }); }
    catch (e) { doclingErr = String(e?.message ?? e); }
    if (docling && docling.markdown.trim()) {
      emit('extracting', { source: 'docling' });
      const title = docling.title || (url ? new URL(url).hostname : 'Document');
      const trimmedMd = docling.markdown.trim();
      const metadata = { ...emptyMetadata(), sourceUrl: url, statusCode, quality: qualityScore(trimmedMd) };
      metadata.contentLength = trimmedMd.length;
      return { markdown: formatHeader(title, url, null) + trimmedMd, title, source: 'docling', metadata };
    }
    // null (unconfigured) or failed → fall through to markitdown
    if (doclingErr) emit('extracting', { source: 'markitdown', note: `docling failed: ${doclingErr}` });
  }
  ...
  if (!comments && !!process.env.MARKITDOWN_YOUTUBE && isYoutubeUrl(url)) {
    ...
    if (yt) {
      ...
      let transcriptStatus = yt.transcriptStatus;
      let trimmedMd = yt.markdown.trim();

      // No caption track to scrape → fall back to downloading the audio and
      // transcribing it locally via yt-dlp + Whisper, when configured.
      if ((transcriptStatus === 'none' || transcriptStatus === 'error') && !!process.env.WHISPER_MEDIA_URL) {
        const whisper = await whisperMediaClient(url, { signal });
        if (whisper && whisper.transcriptStatus === 'ok' && whisper.markdown.trim()) {
          emit('extracting', { source: 'whisper-media' });
          const descIdx = trimmedMd.indexOf('## Transcript');
          const descPart = descIdx >= 0 ? trimmedMd.slice(0, descIdx).trim() : '';
          trimmedMd = [descPart, whisper.markdown.trim()].filter(Boolean).join('\n\n').trim();
          transcriptStatus = 'ok';
        }
      }
      ...
      if (transcriptStatus) result.transcriptStatus = transcriptStatus;
      ...
      if (transcriptStatus === 'blocked' || transcriptStatus === 'error') {
        result.noStore = true;
      }
      return result;
    }
  ...
  // Generic media ingestion via yt-dlp + Whisper: explicit opt-in
  // (?media=whisper) for video/audio URLs from any site yt-dlp supports
  // (Twitter/X, TikTok, Vimeo, SoundCloud, podcast pages, …). YouTube is
  // handled above (with its own caption-track-first path).
  if (!comments && mediaEngine === 'whisper' && !isYoutubeUrl(url)) {
    const media = await whisperMediaClient(url, { signal });
    if (media && media.markdown.trim()) {
      emit('extracting', { source: 'whisper-media' });
      const f = media.fields || {};
      const title = media.title || new URL(url).hostname;
      const trimmedMd = media.markdown.trim();
      const metadata = {
        ...emptyMetadata(),
        sourceUrl: url, statusCode,
      ...
      metadata.ytViews = f.views || null;
      const markdown = formatHeader(title, url, null) + trimmedMd;
      return { markdown, title, source: 'whisper-media', metadata };
    }
    // sidecar down / not a media URL yt-dlp recognizes → fall through
  }
 ...
 * @param {'docling'} [options.engine]           opt-in high-quality/complex-document engine (query param)
 * @param {function} [options.doclingClient]     injectable (tests), defaults to convertViaDocling
 */
export async function extractFile(buffer, options = {}) {
  const {
    ...
    ocrFn = ocrPdf,
    engine,
    doclingClient = convertViaDocling,
  } = options;
  ...
  // table structure, OCR). Falls back to markitdown below on no-sidecar
  // (null) or conversion failure — MarkItDown stays the unconditional default.
  if (engine === 'docling') {
    let docling = null;
    try { docling = await doclingClient(buffer, { filename, contentType }); }
    catch (err) { console.warn('Docling conversion failed, falling back to markitdown:', String(err?.message ?? err)); docling = null; }
    if (docling && docling.markdown.trim()) return fileResult(docling, docling.title || filename || 'Document', filename, 'docling');
  }

  // document (or media/docling with no provider configured) → markitdown sidecar
