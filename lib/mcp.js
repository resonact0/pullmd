...
import { queryExtract } from './query-extract.js';
...
// query-extract: gated on the `query` param (read_url only). MCP has no
// response headers, so a whole-page fallback with no BM25 match is signaled
// in-band via this marker instead of server.js's X-Extracted/X-Extract-*
// headers. Never called when `query` is absent/empty; never applied to
// cached markdown (extraction happens after cache read/put, before the
// frontmatter merge, and its output is never written back to the cache).
const NO_MATCH_MARKER = '<!-- query-extract: no match; returning full page -->\n\n';
...
/**
 * Runs query-extract over `bodyMd` when `query` is present; no-op otherwise.
 *
 * A whitespace-only query is treated as inactive, mirroring server.js's
 * trimmedQuery/queryActive gate: an all-whitespace query is not a real query.
 */
function applyQueryExtract(bodyMd, query, maxTokens) {
  const trimmedQuery = typeof query === 'string' ? query.trim() : '';
  if (!trimmedQuery) return { bodyMd, ex: null };
  const ex = queryExtract(bodyMd, trimmedQuery, { maxTokens });
  const text = ex.extracted === false && ex.confidence === 'low'
    ? NO_MATCH_MARKER + ex.markdown
    : ex.markdown;
  return { bodyMd: text, ex };
}
...
export function createMcpServer({
  ...
  server.registerTool(
    ...
    async ({
      ...
      if (useCache) {
        ...
        if (cached) {
          ...
          if (!reddit || !comments || hasComments) {
            ...
            const { bodyMd, ex } = applyQueryExtract(baseMd, query, max_tokens);
            const fm = frontmatter
              ...
              : '';
            let text = mergeFrontmatter(fm + bodyMd, [
            ...
            if (ex && frontmatter) text = mergeFrontmatter(text, extractFrontmatterFields(ex));
      ...
      const { bodyMd, ex } = applyQueryExtract(result.markdown, query, max_tokens);
      const fm = frontmatter
