...
import { extractTextTesseract } from './ocr-tesseract.js';
...
function isTesseractEnabled() {
  return process.env.PULLMD_OCR_TESSERACT !== 'false';
}

/**
 * Extract text/caption an image. Tries local Tesseract OCR first (free, no
 * API key needed, exact text rather than a model's paraphrase); only calls
 * out to a vision model (Ollama or cloud) when Tesseract finds no legible
 * text — e.g. a photo rather than a screenshot/document.
 * @returns {Promise<{markdown:string, usage:object|null, imageSize:string|null}|null>}
 *          null when Tesseract found nothing AND no vision provider is
 *          configured (caller degrades to EXIF-only metadata).
 */
export async function captionImage(buffer, opts = {}) {
  const { mimetype, fetch: fetchFn = globalThis.fetch, ocrFn = extractTextTesseract } = opts;
  ...
  if (isTesseractEnabled()) {
    let ocrText = null;
    try { ocrText = await ocrFn(buffer, { signal: opts.signal }); } catch { ocrText = null; }
    if (ocrText) return { markdown: `## Extracted text\n\n${ocrText}`, usage: null, imageSize: size };
  }
